...
def test_append_and_read_preserves_order(tmp_path, monkeypatch) -> None:
    ...
    log.log("REVIEW", UNIT_KEY, {"rating": 3})
    log.log("FORGE", UNIT_KEY, {"source": "dictionary"})
    log.log("ENCOUNTER", UNIT_KEY, {"count": 2})

    assert [event.event for event in log.read()] == ["REVIEW", "FORGE", "ENCOUNTER"]
...
def test_reopening_and_appending_preserves_existing_records(tmp_path, monkeypatch) -> None:
    ...
    first = EventLog(path)
    first.log("REVIEW", UNIT_KEY, {"sequence": 1})
    before = path.read_bytes()
    EventLog(path).log("REVIEW", UNIT_KEY, {"sequence": 2})
...
def test_vietnamese_payload_is_readable_utf8(tmp_path, monkeypatch) -> None:
    ...
    EventLog(path).log("FORGE", UNIT_KEY, {"text": "tiếng Việt rất đẹp"})
...
def test_event_type_filtering(tmp_path, monkeypatch) -> None:
    ...
    log.log("FORGE", UNIT_KEY, {})
    log.log("REVIEW", UNIT_KEY, {})

    assert [event.event for event in log.read(event_type="REVIEW")] == [
        "REVIEW",
        "REVIEW",
    ]
...
def test_since_filter_is_inclusive_and_normalizes_offsets(tmp_path, monkeypatch) -> None:
    ...
    log = EventLog(tmp_path / "events.jsonl")
    log.log("REVIEW", UNIT_KEY, {"sequence": 1})
    log.log("REVIEW", UNIT_KEY, {"sequence": 2})
    log.log("REVIEW", UNIT_KEY, {"sequence": 3})
...
def test_stored_timestamp_is_utc_and_day_is_ho_chi_minh(tmp_path, monkeypatch) -> None:
    ...
    event = EventLog(tmp_path / "events.jsonl").log("REVIEW", UNIT_KEY, {})
...
def test_non_dict_payload_is_rejected(tmp_path) -> None:
    with pytest.raises(TypeError, match="payload must be a dict"):
        EventLog(tmp_path / "events.jsonl").log("REVIEW", UNIT_KEY, [])
...
def test_additive_envelope_fields_are_accepted(tmp_path, monkeypatch) -> None:
    ...
    path = tmp_path / "events.jsonl"
    log = EventLog(path)
    expected = log.log("REVIEW", UNIT_KEY, {"rating": 3})
