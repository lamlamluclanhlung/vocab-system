...
def _chunk_match_end(
    text_tokens: tuple[str, ...],
    target_tokens: tuple[str, ...],
    start: int,
) -> int | None:
    """Return the inclusive end of one canonical chunk match at *start*."""
    if text_tokens[start] != target_tokens[0]:
        return None
...
def _frame_match_end(
    text_tokens: tuple[str, ...],
    before_tokens: tuple[str, ...],
    after_tokens: tuple[str, ...],
    start: int,
) -> int | None:
    """Return the inclusive end of one canonical frame match at *start*."""
...
def unit_match_spans(
    text: str,
    unit: str,
    unit_type: str,
) -> tuple[tuple[int, int], ...]:
    """Return canonical inclusive token spans for every matching start."""
    ...
    if unit_type == "chunk":
        ...
        for start in range(len(text_tokens)):
            end = _chunk_match_end(text_tokens, target_tokens, start)
            if end is not None:
    ...
    spans = []
    for start in range(len(text_tokens)):
        end = _frame_match_end(text_tokens, before_tokens, after_tokens, start)
        if end is not None:
            spans.append((start, end))
    return tuple(spans)
...
def contains_unit(text: str, unit: str, unit_type: str) -> bool:
    ...
    return bool(unit_match_spans(text, unit, unit_type))
...
def _has_valid_unit_shape(lemma: str, unit_type: str) -> bool:
    ...
    try:
        unit_match_spans("", lemma, unit_type)
    except ValueError:
        return False
    return True
