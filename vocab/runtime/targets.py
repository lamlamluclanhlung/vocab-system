...
def _resolve_note_id(entry: RegistryEntry, anki: object) -> int:
    query = build_unit_key_query(entry.unit_key)
    with normalized(
        RuntimeTargetDiscoveryError,
        f"note lookup failed for {entry.unit_key}",
        catching=CORPUS_SEAM_SCAN,
    ):
    ...
    if not isinstance(found, list):
        raise RuntimeTargetDiscoveryError(
            f"note lookup for {entry.unit_key} did not return a list"
        )
    for candidate in found:
        if type(candidate) is not int:
            raise RuntimeTargetDiscoveryError(
                f"note lookup for {entry.unit_key} returned a non-integer id"
            )
    if len(set(found)) != len(found):
        raise RuntimeTargetDiscoveryError(
            f"note lookup for {entry.unit_key} returned duplicate ids"
        )
    if not found:
        raise RuntimeTargetDiscoveryError(
            f"no note binds unit_key {entry.unit_key}"
        )
    if len(found) > 1:
        raise RuntimeTargetDiscoveryError(
            f"unit_key {entry.unit_key} is ambiguous across notes {sorted(found)}"
        )
    return found[0]
